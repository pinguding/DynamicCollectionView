...
/// 하위 아이템들을 세로로 배치하는 ``GroupLayout``.
///
/// 너비/높이/간격과 함께 ``ItemLayoutBuilder`` 로 하위 ``ItemLayout`` 을 선언하면
/// 세로 방향 `NSCollectionLayoutGroup` 으로 변환된다.
...
extension VGroupLayout: GroupLayout {
    ...
    /// 하위 아이템들을 세로로 배치한 `NSCollectionLayoutGroup` 을 빌드한다.
    ///
    /// - Returns: 빌드된 세로 방향 `NSCollectionLayoutGroup`.
